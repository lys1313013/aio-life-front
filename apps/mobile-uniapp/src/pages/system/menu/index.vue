<template>
  <view class="container">
    ...
    <view class="list">
      ...
      <view v-else-if="list.length === 0" class="empty">暂无菜单数据</view>
      <view v-else class="list-item" v-for="item in list" :key="item.id">
        <view class="item-content">
          <text class="item-name">{{ item.name }}</text>
          <text class="item-path">路径: {{ item.path }}</text>
          <text class="item-component">组件: {{ item.component || '无' }}</text>
        </view>
...
import { getMenuAdminTreeApi } from '@/api/menu';
...
const flattenTree = (tree: any[]) => {
  ...
  tree.forEach((item) => {
    ...
    if (item.children && item.children.length > 0) {
      res = res.concat(flattenTree(item.children));
    }
  });
  return res;
};
...
const fetchList = async () => {
  try {
    loading.value = true;
    const res = await getMenuAdminTreeApi();
    list.value = flattenTree(res || []);
  } catch (error) {
