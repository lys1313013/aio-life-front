  ...
  <view class="container">
    <view class="form-container">
      <view class="form-item">
        ...
        <input class="input" v-model="formData.title" placeholder="电影/剧集名称" />
      </view>
      ...
      <view class="form-row">
        <view class="form-item half">
          <text class="label">总集数/时长</text>
          <input class="input" v-model.number="formData.totalProgress" type="number" placeholder="24" />
        </view>
        <view class="form-item half">
          <text class="label">当前进度</text>
          <input class="input" v-model.number="formData.currentProgress" type="number" placeholder="8" />
        </view>
      </view>

      <view class="form-item">
        <text class="label">导演</text>
        <input class="input" v-model="formData.director" placeholder="导演" />
      </view>

      <view class="form-item">
        <text class="label">链接</text>
        <input class="input" v-model="formData.url" placeholder="影片链接" />
      </view>
...
const typeOptions = ['电影', '剧集', '动漫'];
const typeIndex = ref(0);
const statusOptions = ['想看', '在看', '看过'];
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      await MovieApi.update(payload);
    } else {
      await MovieApi.save(payload);
    }
...
.bottom-btn {
  ...
  .submit-btn { background-color: #ff9800; color: #fff; border-radius: 44rpx; font-size: 32rpx; &::after { border: none; } }
}
</style>
